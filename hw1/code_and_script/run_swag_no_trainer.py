...
def main():
    ...
    def preprocess_function(examples):
        ...
        answer_paragraphs = [[context[i] for i in example] for example in examples["paragraphs"]]
        # print(xxx)
        # my_list = examples['paragraphs']
        # targets = examples['relevant']
        paragraphs_indexes = examples['paragraphs']
        relevant_indexes = examples['relevant']
        labels = []  # 建立一個空的列表來存儲索引

        # for target in targets:
        #     for sublist in my_list:
        #         if target in sublist:
        #             index = sublist.index(target)
        #             labels.append(index)
        #             break  # 找到目標後退出內循環
        for i in range(len(relevant_indexes)):
            answer = relevant_indexes[i]
            labels.append(paragraphs_indexes[i].index(answer))

        # Flatten out
        first_sentences = list(chain(*questions))
        second_sentences = list(chain(*answer_paragraphs))
